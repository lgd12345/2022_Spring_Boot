...
<div class="container">
		...
		<div>
			<ul>
				<c:forEach items="${filelist}" var="fileInfo">
					<li style="list-style: nome"><c:choose>
							<c:when test="${fileInfo.filetype=='image'}">
								<img
									src="/resources/upload/${fileInfo.savefolder}/${fileInfo.savefile}"
									height="50">
								<%-- <img src="/resources/upload/20220907/${fileInfo.savefolder}/${fileInfo.savefile}" height="50"> --%>
								<%-- <img src="file:///${filePath}/${fileInfo.savefolder}/${fileInfo.savefile}" height="50"> --%>
							</c:when>
							<c:otherwise>
								<img src="/resources/upload/file.png" height="50">
							</c:otherwise>
						</c:choose> ${fileInfo.originfile} <a class="filedown" href="#"
						fno="${fileInfo.fno}" sfolder="${fileInfo.savefolder}"
						ofile="${fileInfo.originfile}" sfile="${fileInfo.savefile}">[다운로드]</a>
					</li>
					<br>
				</c:forEach>
			</ul>
	...
	<div class="form-group">
		<label for="writer">조회수</label> <input type="text"
			class="form-control" id="writer" name="writer"
			value="${board.hitcount}" readonly="readonly">
	</div>
	...
	<div class="container mt-5">
		<div class="form-group">
			...
			<textarea class="form-control" rows="5" id="replyContent"
